...
use super::utils::{extract_file, LauncherError};
use super::Launcher;
...
impl Launcher {
    ...
    async fn install_modded_versions(&mut self) -> Result<(), Box<dyn Error + Send + Sync>> {
        ...
        if self.version.forge.enabled || self.version.neoforge.enabled {
            ...
            // Legacy versions of Forge are packaged differently, requiring custom extraction logic for the installer profile and JAR.
